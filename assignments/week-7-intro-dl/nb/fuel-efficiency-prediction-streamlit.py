...
tpot_results = pd.read_csv('../dat/tpot_res.csv')
dnn_results = pd.read_csv('../dat/dnn_res.csv')
...
with tab2:    
    ...
    with col1:
        ...
        # YOUR CODE GOES HERE!
        if model1_select == 'TPOT':
            tpot_results=tpot_results.rename(columns={'Unnamed: 0':'metrics','0':'TPOT'})
            st.dataframe(tpot_results.T)
        else:
            dnn_results=dnn_results.rename(columns={'Unnamed: 0':'metrics',"0":"DNN"})
            st.dataframe(dnn_results.T)
